...
def get_action(observations):
    ...
    heart_rate = (
        vital_signs_values[HeartRate]
        if vital_signs_time[MeasuredHeartRate] > 0
        else None
    )
    resp_rate = (
        vital_signs_values[RespRate] if vital_signs_time[MeasuredRespRate] > 0 else None
    )
    ...
    if (
        events[18] == 0 and events[19] == 0
    ):  # RadialPulseNonPalpable and HeartSoundsNormal
