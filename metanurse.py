...
def stabilize():
    ...
    for step in range(max_steps):
        ...
        # Ensure required measurements are taken
        for action in required_measurements:
            if action not in actions_taken:
                take_action(action)
                break

        # Address hypotension and unstable tachyarrhythmia immediately
        if vitals["MAP"] is not None and vitals["MAP"] < 60:
            take_action(15)  # GiveFluids
            continue

        if vitals["HR"] is not None and vitals["HR"] > 100:
            take_action(9)  # GiveAdenosine or another appropriate drug
            continue
        ...
        # Breathing and saturation interventions
        ...
        # Finish if all assessments and interventions are complete
