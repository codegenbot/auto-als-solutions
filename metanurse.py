...
def stabilize():
    ...
    for step in range(max_steps):
        ...
        if (vitals["MAP"] is not None and vitals["MAP"] < 20) or (
            ...
            take_action(23)  # Cardiac arrest, start CPR
            continue
        ...
        if vitals["MAP"] is not None and vitals["MAP"] < 60:
            if has_unstable_tachyarrhythmia(events):
                if 24 not in actions_taken:
                    ...
                    take_action(48)  # End (no more viable actions)
            else:
        ...
        if any(events[i] > 0 for i in [4, 5]):  # Vomit or Blood in airway
            take_action(31)  # Use Yankauer Suction Catheter
            continue

        if events[6] > 0:  # Airway blocked by tongue
