...
def choose_action(obs):
    ...
    if obs[4] > 0 and obs[25] == 0:
        return 25  # UseSatsProbe
    if obs[25] > 0 and obs[46] > 0 and obs[52] < 88:
        return 30  # UseNonRebreatherMask
    if obs[16] == 0 and obs[17] == 0:
        return 5  # ExamineCirculation
    if obs[5] > 0 and obs[45] == 0:
    ...
    if obs[28] == 0 or (obs[28] > 0 and obs[46] == 0):
        return 2  # CheckRhythm
    if obs[2] > 0:
        return 16  # ViewMonitor
    if obs[47] > 0 and obs[53] < 8:
        return 29  # UseBagValveMask
    if obs[45] > 0 and obs[51] < 20 or obs[46] > 0 and obs[52] < 65:
