...
def choose_action(obs, state, last_action_time):
    ...
    if state == ResuscitationState.AIRWAY:
        ...
        if obs[7] > 0.5:
            return 29, state, current_time
        return 18, ResuscitationState.BREATHING, current_time

    if state == ResuscitationState.BREATHING:
        if obs[7] > 0.5:
            return 1, ResuscitationState.CPR, current_time
        if obs[11] < 0.5:
        ...
        return 19, ResuscitationState.CIRCULATION, current_time
    ...
    if state == ResuscitationState.CPR:
        ...
        if current_time - last_action_time > 120:
            return 10, state, current_time
        return 17, state, current_time
