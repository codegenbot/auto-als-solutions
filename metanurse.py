...
def stabilize():
    ...
    for step in range(350):
        ...
        def get_vital(index):
            return values[index] if times[index] > 0 else None
        ...
        if all(v in examined_vitals for v in ["airway", "MAP", "Sats", "RR"]) and all(
            v >= t for v, t in zip([vitals["Sats"], vitals["MAP"], vitals["RR"]], [88, 60, 8])
        ):
