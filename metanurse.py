...
def stabilize():
    ...
    def need_measurements():
        return not all(measured(vital_sign) for vital_sign in [25, 27, 16, 4])

    def measured(vital_sign):
        return vital_sign in actions_taken

    def need_measurement_action():
        for action in [25, 27, 16, 4]:
    ...
    for step in range(max_steps):
        ...
        if all(measured(vital_sign) for vital_sign in [25, 27, 16, 4]):  # All required measurements taken
